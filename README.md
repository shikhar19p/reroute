...
| --- | --- | --- |
| `master` | ✅ Active | Primary branch — all fixes applied, CI/CD deploys web automatically |
| `master2` | 🔒 Legacy | Previous working branch, now superseded by master |
| `paymentint` | Payment only | Razorpay integration branch |

Recommended branch for new development: `master`
...
Pushing to `master` automatically triggers a GitHub Actions workflow that:
1. Builds the web bundle (`npx expo export --platform web`)
2. Deploys it to Firebase Hosting at [reroute-aventures.web.app](https://reroute-aventures.web.app)

The workflow config is in `.github/workflows/deploy-web.yml`. Required GitHub Secrets (configured in repo settings):
